...
traits = json.load(open(os.getcwd() + "/json_files/traits.json", "r"))
parts = json.load(open(os.getcwd() + "/json_files/parts.json", "r"))
...
class Part256:
	...
	def __add__(self, obj1):
		...
		for k, v in obj1.probability.items():
			if k in probability:
				probability[k] += v
			else:
				probability[k] = v

		sorted_probability = {k:v for k, v in sorted(probability.items(), key = lambda k: k[1], reverse = True)}
		return sorted_probability

	@property
	def probability(self):
		probability = {}
		if self.dId in probability:
			probability[self.dId] += 37.5
		else:
			probability[self.dId] = 37.5

		if self.r1Id in probability:
			probability[self.r1Id] += 9.375
		else:
			probability[self.r1Id] = 9.375

		if self.r2Id in probability:
			probability[self.r2Id] += 3.125
		else:
			probability[self.r2Id] = 3.125
...
class GetGenes256:
	...
	def market_link(self, stats, breedCount = [0, 0]):
		parts = [self.mouth.dId, self.horn.dId, self.back.dId, self.tail.dId]
		part_string = [f"&part={x}" for x in parts]
		part_string = "".join(part_string)
		s = stats
		del s['__typename']
		stat_string = [f"&{x}={y}&{x}=61" for x, y in s.items()]
		stat_string = "".join(stat_string)
		class_string = f"?class={self.cls.title()}"
		breed_string = f"&breedCount={breedCount[0]}&breedCount={breedCount[1]}"
		market_string = f"{class_string}{stat_string}{part_string}{breed_string}"
	...
	def __init__(self, hex_value):
		...
		self.tail = Part256(self.geneBin.tail, Tail())
