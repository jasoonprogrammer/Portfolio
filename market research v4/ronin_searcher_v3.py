...
ronin_txt = open("txt_files/ronin.txt")
ronin_list = [x.strip().replace("ronin:", "0x") for x in ronin_txt.readlines()]
ronin_txt.close()
axie_list = []
axie_count = 0
...
for ronin in ronin_list:
	print(f"Aquiring axie details from {ronin}")
	while True:
		try:
			time.sleep(maxsleep)
			data = start_ownerquery(ronin, index = 0)
			break
		...
		try:
			...
			if total_axies <= 0:
				...
				for i in range(loops):
					while True:
						try:
							print(f"Aquiring details from axie {i * 100} - {(i + 1) * 100}")
							data = start_ownerquery(ronin, index = i * 100)
							time.sleep(maxsleep - 1)
							break
						except ConnectionError:
							time.sleep(1)
							print("Server Connection Error. Retrying")
	...
	if d_r1 and axie['breedCount'] == 0:
		...
		while True:
			try:
				...
				time.sleep(maxsleep)
				print("Error when running query. retrying")
				if retries >= maxretries:
					total = 0
					print("Max retries! No axies found")
					break
		...
		if total != 0:
			...
			for i in range(0, loops):
				...
				while True:
					try:
						qq = start_marketquery(classes = [axie['class']], parts = parts, hp = hp, morale = morale, skill = skill, speed = speed, index = i * 30, size = 100)
						axies = qq['data']['axies']['results']
						total = qq['data']['axies']['total']
						print(total, "total")
						print(f"Searching through {i * 100} - {(i + 1) * 100}")
						time.sleep(maxsleep)
						break
					except TypeError:
				...
				for i, _axie in enumerate(axies):
					print(i, _axie['genes'])
					...
					if _axie['auction']['seller'] == _axie['owner']:
						if d_r1:
							if ((match_perfect_mouth >= main_axie_perfect_mouth) and (match_perfect_horn >= main_axie_perfect_horn) and (match_perfect_back >= main_axie_perfect_back) and (match_perfect_tail >= main_axie_perfect_tail)):
								...
								print("PERFECT! MAIN AXIE ID# ", axie['id'], "MATCH AXIE ID# ", _axie['id'])
